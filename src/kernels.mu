...
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    // MUSA equivalent of __shfl_down_sync
    input += __musa_shfl_down_sync(0xFFFFFFFF, input, offset);
  }
  return input;
}
...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols) {
  ...
  RUNTIME_CHECK(musaMalloc(&d_input, input_size * sizeof(T)));
  RUNTIME_CHECK(musaMalloc(&d_result, sizeof(T)));

  RUNTIME_CHECK(musaMemcpy(d_input, h_input.data(), input_size * sizeof(T), musaMemcpyHostToDevice));
  RUNTIME_CHECK(musaMemset(d_result, 0, sizeof(T)));

  size_t blockSize = 256;
  size_t gridSize = (diag_len + blockSize - 1) / blockSize;
  // Calculate shared memory size needed for reduction
  ...
  T result;
  RUNTIME_CHECK(musaMemcpy(&result, d_result, sizeof(T), musaMemcpyDeviceToHost));
...
__global__ void flashAttention64(
    ...
    for (int kv_blk = 0; kv_blk < (kv_len + Bc - 1) / Bc; kv_blk++) {
        ...
        if (tid < Br) {
            for (int k = 0; k < Bc; k++) {
                float dot = 0;
                for (int d = 0; d < D; d++) dot += float(sQ[tid * D_PAD + d]) * float(sK[k * D_PAD + d]);
                sS[tid * Bc + k] = dot;
            }
        }
        __syncthreads();
            ...
            if (gq < q_len) {
                ...
                for (int k = 0; k < Bc; k++) {
                    ...
                    if (s > -INFINITY) {
                        ...
                        row_sum += p;
                        for (int d = 0; d < D; d++) acc[d] += p * float(sV[k * D_PAD + d]);
                    }
                }
    ...
    if (tid < Br) {
        ...
        if (gq < q_len) {
            ...
            for (int d = 0; d < head_dim; d++) op[d] = T(acc[d] * inv_l);
        }
    }
}

template <typename T>
__global__ void flashAttention128(
    const T* __restrict__ Q, const T* __restrict__ K, const T* __restrict__ V, T* __restrict__ O,
    int q_len, int kv_len, int num_q_heads, int num_kv_heads, int head_dim, float scale, bool causal
) {
    constexpr int D = 128, D_PAD = D + PAD, Br = 16, Bc = 16;
...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
    ...
    RUNTIME_CHECK(musaMemcpy(d_q, h_q.data(), h_q.size() * sizeof(T), musaMemcpyHostToDevice));
    RUNTIME_CHECK(musaMemcpy(d_k, h_k.data(), h_k.size() * sizeof(T), musaMemcpyHostToDevice));
    RUNTIME_CHECK(musaMemcpy(d_v, h_v.data(), h_v.size() * sizeof(T), musaMemcpyHostToDevice));

    float scale = 1.0f / sqrtf(float(head_dim));

    if (head_dim <= 64) {
        constexpr int Br = 32, Bc = 32, D = 64, D_PAD = D + PAD;
        dim3 grid(batch_size, query_heads, (target_seq_len + Br - 1) / Br);
        dim3 block(32);
        size_t smem = (Br * D_PAD + 2 * Bc * D_PAD) * sizeof(T) + Br * Bc * sizeof(float);
        flashAttention64<T><<<grid, block, smem>>>(d_q, d_k, d_v, d_o, target_seq_len, src_seq_len, query_heads, kv_heads, head_dim, scale, is_causal);
    } else {
        constexpr int Br = 16, Bc = 16, D = 128, D_PAD = D + PAD;
        dim3 grid(batch_size, query_heads, (target_seq_len + Br - 1) / Br);
        dim3 block(16);
        size_t smem = (Br * D_PAD + 2 * Bc * D_PAD) * sizeof(T) + Br * Bc * sizeof(float);
        flashAttention128<T><<<grid, block, smem>>>(d_q, d_k, d_v, d_o, target_seq_len, src_seq_len, query_heads, kv_heads, head_dim, scale, is_causal);
    }
