...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
    ...
    if (head_dim <= 64) {
        ...
        flashAttention64<T><<<grid, block, smem>>>(d_q, d_k, d_v, d_o, q_len, kv_len, q_heads, kv_heads, head_dim, scale, causal);
    } else {
        constexpr int Br = 16, Bc = 16, D = 128, D_PAD = D + PAD;
        dim3 grid(batch, q_heads, (q_len + Br - 1) / Br);
        dim3 block(16);
        size_t smem = (Br * D_PAD + 2 * Bc * D_PAD) * sizeof(T) + Br * Bc * sizeof(float);
        flashAttention128<T><<<grid, block, smem>>>(d_q, d_k, d_v, d_o, q_len, kv_len, q_heads, kv_heads, head_dim, scale, causal);
    }
